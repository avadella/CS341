...
{
	//Initalize node relations w/ values
	...
	//ending recursive nature using if statements
	//The parent of root is null, end of recursion
	if(newNode->getParent() == nullptr)
	{
		BinarySearchTree::setRoot(newNode); //If newnode has no parent, must be root
		return;
	}
	...
	//The parent node of newNode is red since it gets past above if statements
	//Parent is also the root, meaning no grandparent relation exists yet!
	...
	{
		//At this point, enough nodes exist for there to be a grandparent relation
	...
	if(parent == grandparent->getRightChild())
	{
		...
		//aunt is black, and recoloring/rotating must be done to keep black nodes balanced
		else
		{
	...
	{
		//std::cout << "aunt is Right" << std::endl;
...
void RedBlackTree::rotateLeft(TreeNode *& root, TreeNode *& newNode)  //x is root, y is child
{
	...
	{
		//In the condition where tree is in a "line" instead of a "tree"
